...
import com.kangren.cpr.viewModel.helper.*;
import com.kangren.draw.*;
...
@SuppressLint("NewApi") public class CprFragment  extends Fragment implements OnClickListener  {
	...
	private DrawPrintForPress printPress;
	  ...
	  private OnePressCycle pressCycle,lastCycle;//按压一次
	  
	  private OneBlowCycle blowCycle;//吹气一次
	  //总数 当前 流程
	  private CprOneCycleData totalData;

	  private Language lan;
	  private  CprOneCycleData cData,lastData;
	  private List<CprOneCycleData> history;//历次操作详情
				...
				public void pressUpdate(CprOneCycleData currentData, final SoundType st) {
					...
					ac.runOnUiThread(new Runnable() {
						...
						public void run() { 
							...
							//判断自动结束，先吹后按 ，实战当前循环为5 吹气为2 ，考核为正确2
				...
				public void blowUpdate(CprOneCycleData currentData, final SoundType st) {
					...
					addTotalData();
						...
						public void run() {
							...
							String st="javascript:setdata('"+JsonUtil.ToJson(cData)+"','"+JsonUtil.ToJson(totalData)+"')";
							cprWebView.loadUrl(st);
							//判断自动结束，先按后吹 ，实战当前循环为5 吹气为2 ，考核为正确2
	    	...
	    	iReceiver=new IReceiver() {
				...
				public void press(PressReceiveMessage press ) {
					...
					//回弹到0 只有状态切换过 ，才更新UI
					 if(pressCycle.startNum>-1&&press.num==0&&!press.isIn){
						 ...
						 //之前的频率算法	 pinglv=(int) (60*1000/(pressCycle.endTime-pressCycle.startTime))*0.75;
						 if(pinglv>cprSetting.anyapinlvMax)
							 pinglv-=15;
						 if(pinglv<cprSetting.anyapinlvMin)
							 pinglv+=15;
				...
				public void palyBackCompeled() {
					...
					ac.runOnUiThread(new Runnable() {
						...
						@Override
						public void run() {
							//确保是在开始的状态， 极端情况，开始调用时，已经被手动点停止， 或者别的原因，就是确保完成时， 是停止。//结局overflowstatckerror
